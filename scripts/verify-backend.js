...
async function run() {
  ...
  try {
    ...
    const twilioInbound = await requestJson(baseUrl, "/api/webhooks/twilio/whatsapp", {
      method: "POST",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded"
      },
      body: "From=whatsapp%3A%2B27824440101&WaId=27824440101&ProfileName=Verification%20Agent&Body=Testing%20reply%20from%20Twilio&MessageSid=SMVERIFY123"
    });
    const operationsAfterWhatsapp = await requestJson(baseUrl, "/api/admin/operations", {
      headers: { cookie }
    });
    ...
    const failures = [
      ...
      !twilioInbound.ok && `/api/webhooks/twilio/whatsapp returned ${twilioInbound.status}`,
      ...
      typeof operationsAfterLead.body?.snapshot?.whatsapp?.metrics?.manualReady !== "number" &&
        "WhatsApp metrics missing manual-ready count",
      typeof operationsAfterLead.body?.snapshot?.whatsapp?.metrics?.sendFailed !== "number" &&
        "WhatsApp metrics missing failed-send count",
      typeof operationsAfterLead.body?.snapshot?.whatsapp?.metrics?.totalOutbox !== "number" &&
        "WhatsApp metrics missing total outbox count",
      !operationsAfterWhatsapp.body?.snapshot?.whatsapp?.threads?.some((thread) =>
        thread.messages?.some((message) => message.direction === "inbound" && /Testing reply from Twilio/.test(message.body || ""))
      ) && "Twilio-style inbound webhook did not land in the WhatsApp thread log",
